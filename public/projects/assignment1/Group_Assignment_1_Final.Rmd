...
```{r download data, echo=FALSE, cache=TRUE}
...
election2020_results <- vroom("countypres_2000-2020.csv") %>% 
  janitor::clean_names() %>% 
...
The plan is to setup 3 dataframes for population, vaccination rate and election results and ultimately merging the dataframe by using the fips variable.\n

First, a new dataframe needs to be created to only include Donald Trump votes. Subsequently, the no. of votes needs to be expressed as a percentage.
...
#As we lost the total votes, we need to add them back again by using left_join().

election_unique_join <- unique(left_join(election_unique, 
...
Now that we have the percentage of votes for Trump per county, we need to create a dataframe that only contains the county and percentage of population being fully vaccinated on September 2 (last available date).
...
We now have all dataframes and simply need to merge them into one using left_join().
```{r population per county}
#Merge vaccination and trump votes
vaccination_vote <- left_join(vaccinations_fips, election_trump_filtered, 
                              by="fips", copy=FALSE)

#Add population numbers
vacc_vote_pop <- left_join(vaccination_vote, population, by="fips", copy=FALSE)

#Omit all rows with any NA values
vacc_vote_pop <- na.omit(vacc_vote_pop)
```

As the dataframe is put together, the graph can now be constructed.
...
ggplot(vacc_vote_pop, aes(x=vote_percentage, y=series_complete_pop_pct, 
  ...
  annotate("text", x=0.5, y=0.97, label="EVERY U.S. COUNTY", color="black", size=8)+
...
-   What, if anything, gave you the most trouble: Producing some graphs
